...
class UnsupervisedRunner(BaseRunner):
    ...
    def evaluate(self, epoch_num=None, keep_all=True):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            target_mask = target_masks.cpu().detach().numpy()

            if target_mask_total.size>0:
                target_mask_total = np.concatenate((target_mask_total, target_mask))
            else:
                target_mask_total = target_mask.copy()

            loss = self.loss_module(predictions, targets, target_masks)  # (num_active,) individual loss (square error per element) for each active value in batch
            batch_loss = torch.sum(torch.sqrt(loss)).cpu().item()
