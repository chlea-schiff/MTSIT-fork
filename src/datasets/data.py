...
class PandasTSData(BaseData):
    ...
    def load_all(self, root_dir, file_list=None, pattern=None):
        ...
        data_paths = [p for p in selected_paths if os.path.isfile(p) and "_inputs.pickle" in p]
        label_paths = [p for p in selected_paths if os.path.isfile(p) and "_labels.pickle" in p]
    ...
    def load_single(self, data_path, label_path):
        ...
        """ for labels """
        lengths = labels_df.applymap(lambda x: len(x)).values  # (num_samples, num_dimensions) array containing the length of each series
        horiz_diffs = np.abs(lengths - np.expand_dims(lengths[:, 0], -1))

        # most general check: len(np.unique(lengths.values)) > 1:  # returns array of unique lengths of sequences
        if np.sum(horiz_diffs) > 0:  # if any row (sample) has varying length across dimensions
            logger.warning("Not all time series dimensions have same length - will attempt to fix by subsampling first dimension...")
            labels_df = labels_df.applymap(subsample)  # TODO: this addresses a very specific case (PPGDalia)

        if self.config['subsample_factor']:
            labels_df = labels_df.applymap(lambda x: subsample(x, limit=0, factor=self.config['subsample_factor']))

        lengths = labels_df.applymap(lambda x: len(x)).values
        vert_diffs = np.abs(lengths - np.expand_dims(lengths[0, :], 0))
        if np.sum(vert_diffs) > 0:  # if any column (dimension) has varying length across samples
            self.max_seq_len = int(np.max(lengths[:, 0]))
            logger.warning("Not all samples have same length: maximum length set to {}".format(self.max_seq_len))
        ...
        labels_df = pd.concat((pd.DataFrame({col: labels_df.loc[labels_df.index[row], col] for col in labels_df.columns}).reset_index(drop=True)
                        .set_index(pd.Series(lengths[row, 0]*[labels_df.index[row]])) for row in range(labels_df.shape[0])), axis=0)
